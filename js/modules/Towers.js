...
module.exports = {
    ...
    retrieveTowers: function(client) {
        ...
        let sites = []; // Initialize network sites array
        client.getAll.networkSites()
            .then(async json => {
                if(json.error) { // Check for error
                    console.error(json.error);
                } 
                else if(json.paginator.total_pages > 1) { // Check for more network sites
                    sites.push.apply(sites, json.data); // Push current sites 
                    let page = json.paginator.current_page + 1;
                    for(let i = page; i === json.paginator.total_pages; i++) {
                        json = await client.getAll.networkSites({ page: i });
                        sites.push.apply(sites, json.data);
                    }
                    populateTowers(sites);
                } 
                else {
    ...
    /**
     * Create tower optoins for selection.
     */
